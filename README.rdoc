...
Additional Ruby gems used are XXX

Default database is PostgreSQL (for Heroku deployment).

First make sure your PostgreSQL database is up and configured properly. Then create the application database with `rake db:setup`.

To start the web server use `rails server`.

At the moment all the users are regular users by default. To promote the user to an admin role use `rails console` and update the users admin parameter from `false`to `true`.
